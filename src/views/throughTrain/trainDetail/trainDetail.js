...
export default {
    ...
    methods: {
        ...
        saveSetting() {
            ...
            this.$post(`/admin/throughtrain/edit`, params).then(res => {
                if (res.code === 200) {
                    if (res.data) {
                        ...
                        params.price = (Number(params.price) / 100).toFixed(2)
                        params.unit_price = (Number(params.unit_price) / 100).toFixed(2)
                    }
                } else {
        ...
        // 获取班次信息
        getTimers() {
            this.$fetch(`/admin/throughtrain/get`, { id: this.$route.query.id }).then(res => {
                let data = res.data
                console.log(123)
                console.log(data)
                let { title, details, project, image, price, map, id,unit_price } = data
                let projectInfo = { title, details, project, image, price, map, id,unit_price }
                projectInfo.price = (Number(projectInfo.price) / 100).toFixed(2)
                projectInfo.unit_price = (Number(projectInfo.unit_price) / 100).toFixed(2)
                this.projectInfo = projectInfo
                this.img_json = JSON.parse(data.img_json)
